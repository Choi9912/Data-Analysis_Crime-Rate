...
plt.title('2021 전국 범죄건수 조사', size=15)
plt.pie(crime, labels=label, shadow=True, autopct='%.1f%%', startangle = 90, textprops = {'fontsize':8})
plt.show()
...
label = new_df['지역']
plt.plot(label,crime_rate,color='b',linestyle='--',marker='o')
plt.xticks(size=10, rotation=90)
plt.title('2021 인구대비 전국 범죄율 조사', size=15)
...
plt.title('2021 경기도 북부 인구대비 4대 범죄 범죄율', size=15)
...
plt.title('2021 경기도 남부 인구대비 4대 범죄 범죄율', size=15)
...
plt.bar(['의정부시','평택시'],crime_rate,width=0.3)
plt.title('2021 경기도 인구대비 4대 범죄 범죄율', size=12)
...
crimerate = pd.DataFrame({'지역':north_label+south_label,
                             '인구 대비 범죄율':north_crimerate + south_crimerate})

#인구 대비 범죄율 내림차순 정렬
crimerate = crimerate.sort_values('인구 대비 범죄율', ascending = False)
...
plt.figure(figsize=(15,10))
plt.bar(crimescene_label,crimescene_sum)
plt.xticks(size=10,rotation=90)
plt.title('2021 경기도 발생장소별 4대 범죄 건수 조사', size=15)
...
plt.title('2021 경기도 발생장소별 4대 범죄 건수 조사', size=15)    
plt.pie(crimescene['건수'], labels=crimescene.index, shadow=True, startangle = 90, autopct='%.1f%%', textprops = {'fontsize':9})
plt.show()
...
plt.bar(number_df['동'],number_df['세대수'])
plt.xticks(size=10,rotation=90)
plt.xlabel('동')
plt.ylabel('세대수')
...
pt_119.plot(ax=ax, marker='v', color='red', label='안심 비상벨')
pol2.plot(ax=ax, marker='s', color='darkblue', label='경찰서')

ax.set_title("비전동", fontsize=20)
ax.set_axis_off()
plt.legend()
plt.show()
...
folium.Choropleth(geo_data=geo_datam,fill_opecity).add_to(map_osm)
...
for i in range(0,len(new_emergency_alram.index)):
    ...
    marker= folium.Circle([lat,lng], popup=name, radius = 200, color='red').add_to(map_osm)    
...
new_pol = pd.DataFrame({'위치':new_location,
                                '위도':new_lat,
                                '경도':new_lng})
...
for i in range(0,len(new_pol.index)):
    ...
    marker= folium.Circle([lat,lng], popup=name, radius = 600, color='red').add_to(map_osm)    
...
ax = set.plot(column="EMD_KOR_NM", figsize=(8,8), alpha=0.8)
pt_119.plot(ax=ax, marker='v', color='red', label='기존 안심 비상벨')
pol2.plot(ax=ax, marker='v', color='darkblue', label='기존 경찰서')
new_emergency_alram.plot(ax=ax, marker='v', color='purple', label='신규 안심 비상벨')
new_pol.plot(ax=ax, marker='v', color='green', label='신규 경찰초소')
...
for i in range(0,len(new_emergency_alram.index)):
    ...
    lat = new_emergency_alram.loc[i, '위도']
    lng = new_emergency_alram.loc[i, '경도']
    marker= folium.Marker([lat,lng],icon=folium.Icon(color='purple')).add_to(map_osm) 
...
for i in range(0,len(new_pol.index)):
    ...
    lat = new_pol.loc[i, '위도']
    lng = new_pol.loc[i, '경도']
    folium.Marker([lat,lng], icon=folium.Icon(color='green')).add_to(map_osm)
